...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    for part in dev ${train_set}; do
        ...
        echo "multi" > "${target_dir}/audio_format"
    done
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    for part in dev_clean test_clean dev_other test_other; do
        target_dir="${target_dumpdir}/raw/${part}_first"
        echo "Appending first utterance: dump/raw/${part} -> ${target_dir}"

        cp -a "dump/raw/${part}" "${target_dir}"
        python local/append_first_utt.py --file_path "${target_dir}/wav.scp"
