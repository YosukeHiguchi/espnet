...
# HC-CTC related (e.g., bpe_unigram256,bpe_unigram2048,bpe_unigram16384)
asr_units=
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ -z ${asr_units} ]; then
            ...
            log "Multi-target ASR"
            unit_list=(${asr_units//,/ })
            for u in ${unit_list[@]}; do
                u_bpemodel=${token_listdir}/${u}/bpe.model
                u_token_type=${u::4}
                u_token_type=${u_token_type/_/}
                u_token_list=${token_listdir}/${u}/tokens.txt

                if [ ! -f ${u_bpemodel} ]; then
                    log "Error: ${u_bpemodel} does not exist"
                    exit 2
                fi

                if [ ! -f ${u_token_list} ]; then
                    log "Error: ${u_token_list} does not exist"
                    exit 2
                fi

                _opts+="--bpemodels ${u_bpemodel} "
                _opts+="--token_types ${u_token_type} "
                _opts+="--token_lists ${u_token_list} "
            done
            ...
            unit_idx=0
            for u in ${unit_list[@]}; do
                u_name=text${unit_idx}_${u}
                _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text,${u_name},text "
                unit_idx=$((unit_idx+1))
            done
            _opts+="--train_shape_file ${asr_stats_dir}/train/speech_shape "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_shape.${token_type} "
            ...
            for u in ${unit_list[@]}; do
                u_name=text${unit_idx}_${u}
                _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/text,${u_name},text "
                unit_idx=$((unit_idx+1))
            done
            _opts+="--valid_shape_file ${asr_stats_dir}/valid/speech_shape "
            _opts+="--valid_shape_file ${asr_stats_dir}/valid/text_shape.${token_type} "
